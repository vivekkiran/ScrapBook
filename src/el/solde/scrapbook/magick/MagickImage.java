...
import el.solde.scrapbook.fakeawt.Dimension;
import el.solde.scrapbook.fakeawt.Rectangle;

/**
 * Encapsulation of the ImageMagick Image object. We use MagickImage here so as
 * not to be confused with java.awt.Image.
 *
 * @author Eric Yeo
 */
public class MagickImage extends Magick {
	...
	/**
	 * Internal ImageMagick Image handle. We use long (64-bits) for portability.
	 */
	...
	/**
	 * Construct a MagickImage that is made up of all the images in the
	 * specified array. If any of the images contains multiple frames, the
	 * frames are also appended to the new image. All the images are cloned.
	 * 
	 * @param images
	 *            array of images to linked
	 */
	...
	/**
	 * Helper for the constcutor to create an image that is made up of all the
	 * images in the specified array. If any of the images contains multiple
	 * frames, the frames are also appended to the new image. All the images are
	 * cloned.
	 * 
	 * @param images
	 *            array of images to linked
	 * @throws MagickException
	 *             if the new image cannot be constructed
	 * @see MagickImage
	 */
	 ...
	 *
	 * @param imageInfo
	 *            the ImageInfo for an image file to read
	 ...
	 *            the ImageInfo for an image file to read
	 * @see <a
	 *      href="http://www.imagemagick.org/api/constitute.php#PingImage">The
	 *      underlying ImageMagick call</a>
	 */
	 ...
	 *
	 * @param imageInfo
	 *            the ImageInfo instance for default settings, etc
	 * @param blob
	 *            the image to be read in memory
	 *
	 * @exception MagickException
	 *                on error
	 */
	 ...
	 *
	 * @param imageInfo
	 *            specifies the parameters for the blank image
	 */
	 ...
	 * @see <a
	 *      href="http://www.imagemagick.org/api/constitute.php#ReadImage">The
	 *      underlying ImageMagick call</a>
	 * @exception MagickException
	 *                on error
	 */
	 ...
	 *
	 * @param filter
	 *            the filter type from constants defined in the class FilterType
	 * @see FilterType
	 * @exception MagickException
	 *                on error
	 */
	public native void setFilter(int filter) throws MagickException;
	 ...
	 *
	 * @param noiseType
	 *            The type of noise: Uniform, Gaussian, Multiplicative, Impulse,
	 *            Laplacian, or Poisson.
	 * @see NoiseType
	 * @return An image with additional noise.
	 * @see <a
	 *      href="http://www.imagemagick.org/api/effect.php#AddNoiseImage">The
	 *      underlying ImageMagick call</a>
	 * @exception MagickException
	 *                on error
	 */
	 ...
	 *
	 * @param channel
	 *            The channel(s) to which the blurring should apply (see
	 *            ChannelType)
	 * @param raduis
	 *            The radius of the gaussian, in pixels, not counting the center
	 *            pixel
	...
	public native MagickImage blurImageChannel(int channel, double raduis,
			double sigma) throws MagickException;
	...
	/*
	 * Annotates an image with test. Optionally the annotation can include the
	 * image filename, type, width, height, or scene number by embedding special
	 * format characters.
	 * 
	...
	/**
	 * Surrounds the image with a border of the color defined by the border
	 * color member of the image structure. The width and height of the border
	 * are defined by the corresponding members of the Rectangle.
	 *
	 * @param borderInfo
	 *            the rectangle for which border is drawn
	...
	/**
	 * Creates a new image that is a copy of an existing one with the edges
	 * highlighted, producing a 'charcoal-drawing' effect.
	 *
	 * @param raduis
	 ...
	 * raiseInfo define the width of the vertical and horizontal edge of the
	 * effect.
	 *
	 * @param raiseInfo
	 *            the rectangle for which border is drawn
	 * @param raise
	 *            true to create raise effect, false to lower
	 ...
	 *
	 * @param chopInfo
	 *            the rectange to chop the image
	 * @exception MagickException
	 *                on error
	 */
	 ...
	 *
	 * @param opacity
	 *            string percentage value for opacity
	 * @param target
	 *            a colour value
	 * @exception MagickException
	 *                on error
	 ...
	 * 
	 * @see <a
	 *      href="http://www.imagemagick.org/script/command-line-options.php#compose">Command
	 *      line explanation</a>
	 * @see <a href="http://www.imagemagick.org/api/composite.php">The
	 *      underlying ImageMagick call</a>
	 * @exception MagickException
	 *                on error
	 */
	 ...
	 *                on error
	 */
	public native boolean contrastImage(boolean sharpen) throws MagickException;

	/**
	 * Returns a copy of all fields of the input image. The the pixel memory is
	 * allocated but the pixel data is copy is optional.
	 * 
	 ...
	 * @param pixels
	 *            the raw image in an array of pixels
	 * @see <a
	 *      href="http://www.imagemagick.org/api/constitute.php#ConstituteImage">The
	 *      underlying ImageMagick call</a>
	 * @exception MagickException
	 *                on error
	 */
	public native void constituteImage(int width, int height, String map,
			byte[] pixels) throws MagickException;
	 ...
	 */
	public native void constituteImage(int width, int height, String map,
			float[] pixels) throws MagickException;
	 ...
	 *                on error
	 */
	public native boolean drawImage(DrawInfo aInfo) throws MagickException;
	...
	/**
	 * Creates a new image that reflects each scanline in the vertical
	 * direction.
	 * 
	...
	/**
	 * Creates a new image that reflects each scanline in the horizontal
	 * direction
	 * 
	 ...
	 * operator - and the amount of blurring - is controlled by selecting a
	 * 'radius' and a 'sigma'. The radius sets the circle around each pixel of
	 * the 'neighborhood' used for calculating an average pixel color. Sigma
	 * determines how much the calculated average affects the pixel.
	 *
	 * @param raduis
	 *            The radius of the Gaussian, in pixels, not counting the center
	 *            pixel
	 ...
	 *
	 * @param amount
	 *            Amount of implosion if positive, explosion if negative.
	 *
	 ...
	 *                on error
	 */
	public native boolean matteFloodfillImage(PixelPacket target, int matte,
			int x, int y, int method) throws MagickException;
	 ...
	 *            The draw info
	 * @param target
	 *            The RGB value of the target colour
	 * @param x
	 *            the starting x location of the operation
	 * @param y
	 *            the starting y location of the operation
	 * @param method
	 *            either FloodfillMethod or FilltoBorderMethod
	...
	public native boolean colorFloodfillImage(DrawInfo drawInfo,
			PixelPacket target, int x, int y, int method)
			throws MagickException;

	/**
	 * Applies a digital filter that improves the quality of a noisy image. Each
	...
	/**
	 * Normalizes the pixel values to span the full range of color values. This
	 * is a contrast enhancement technique.
	 * 
	 * @return a boolean value to indicate success
	 * @exception MagickException
	 *                on error
	...
	/**
	 * This operation attempts to reduce the 'noise' in the image. This can be
	 * used to eliminate minor dust and scratches from scanned images.
	 * <p>
	 * The principal function of the noise peak elimination filter is to smooth
	 ...
	 * replace a pixel with its next neighbor in value in a circular
	 * neighborhood if this pixel has been found to be noise. A pixel is defined
	 * as noise if the pixel is a minimum or maximum within the neighborhood.
	 *
	 * @param radius
	 *            The radius of the pixel neighborhood.
	 *
	...
	/**
	 * Converts the reference image from RGB to an alternate colorspace. The
	 * transformation matrices are not the standard ones: the weights are
	 * rescaled to normalized the range of the transformed values to be
	 * [0..MaxRGB].
	 * 
	 * @param colorspace
	 *            the target colorspace to transform to
	 * @return a boolean value to indicate success
	 * @exception MagickException
	 *                on error
	 */
	 ...
	 * 
	 * @param xOffset
	 *            An integer that specifies the number of columns \ to roll in
	 *            the horizontal direction
	 * @param yOffset
	 *            An integer that specifies the number of rows to \ roll in the
	 *            vertical direction
	...
	/**
	 * Creates a new image that is a scaled size of an existing one using pixel
	 * sampling.
	 * 
	 * @param cols
	 *            An integer that specifies the number of columns in \ the
	 *            sampled image
	 * @param rows
	 *            An integer that specifies the number of rows in the \ sampled
	 *            image
	...
	/**
	 * Return a new image that is a scaled version of the original. To scale a
	 * scanline from x pixels to y pixels, each new pixel represents x/y old
	 * pixels. To read x/y pixels, read (x/y rounded up) pixels but only count
	 * the required fraction of the last old pixel read in your new pixel. The
	 * remainder of the old pixel will be counted in the next new pixel.
	 * 
	 * @param cols
	 *            An integer that specifies the number of columns in \ the
	 *            scaled image
	 * @param rows
	 *            An integer that specifies the number of rows in the \ scaled
	 *            image
	 ...
	 * identifying units that are homogeneous using the fuzzy c-means technique.
	 *
	 * @param colorspace
	 *            A {@link ColorspaceType} value that indicates the colorspace.
	 *            Empirical evidence suggests that distances in YUV or YIQ
	 *            correspond to perceptual color differences more closely than
	 *            do distances in RGB space. The image is then returned to RGB
	 *            colorspace after color reduction.
	 * @param cluster_threshold
	 *            Specify cluster threshold as the number of pixels in each
	 *            cluster must exceed the the cluster threshold to be considered
	 *            valid.
	 * @param smoothing_threshold
	 *            Smoothing threshold eliminates noise in the second derivative
	 *            of the histogram. As the value is increased, you can expect a
	 *            smoother second derivative. The default is 1.5.
	 *
	...
	public native int segmentImage(int colorspace, double cluster_threshold,
			double smoothing_threshold) throws MagickException;
	 ...
	 *
	 * @param threshold
	 *            Ranges from 0 to MaxRGB and is a measure of the extent of the
	 *            solarization.
	 *
	 * @exception MagickException
	 *                on error
	 */
	public native void solarizeImage(double threshold) throws MagickException;
	...
	/**
	 * A special effects method that randomly displaces each pixel in a block
	 ...
	 *                on error
	 */
	public native MagickImage spreadImage(int radius) throws MagickException;
	 ...
	 *
	 * @param degrees
	 *            Defines the tightness of the swirling effect.
	 *
	 * @return A new, swirled, image.
	 * @exception MagickException
	 *                on error
	 */
	public native MagickImage swirlImage(double degress) throws MagickException;

	/**
	 * Initializes the red, green, and blue intensities of each pixel as defined
	 * by the colormap index.
	 * 
	 * @exception MagickException
	 *                on error
	 */
	...
	/**
	 * Creates a new image that is a transformed size of of existing one as
	 * specified by the crop and image geometries.
	 *
	 * If a crop geometry is specified a subregion of the image is obtained. If
	 * the specified image size, as defined by the image and scale geometries,
	 * is smaller than the actual image size, the image is first minified to an
	 * integral of the specified image size with an antialias digital filter.
	 * The image is then scaled to the exact specified image size with pixel
	 ...
	 *
	 * @param cropGeometry
	 *            a crop geometry string. This geometry \ defines a subregion of
	 *            the image.
	 * @param imageGeometry
	 *            a image geometry string. The specified \ width and height of
	 *            this geometry string are absolute.
	 * @exception MagickException
	 *                on error
	 */
	...
	/**
	 * Converts the reference image from an alternate colorspace. The
	 * transformation matrices are not the standard ones: the weights are
	 * rescaled to normalized the range of the transformed values to be
	 * [0..MaxRGB].
	 *
	 * @param colorspace
	 *            An unsigned integer value defines which \ colorspace to
	 ...
	 *            The color to search for in the image
	 * @param opacity
	 *            The opacity of the transparent image
	 * @return a boolean value indicating success
	 * @exception MagickException
	 *                on error
	 */
	...
	/**
	 * Creates a new image that is a copy of an existing one with the pixels
	 * sharpened using an "unsharp" masking technique.
	 * <p>
	 * This process starts by building a temporary, {@link #gaussianBlurImage
	 * blurred}, copy of the image. Then each pixel in this "unsharp" image is
	 * compared against its corresponding pixel in the original image. If their
	 * difference is above a threshold, a percentage of the difference is added
	 * back into the original pixel.
	 * <p>
	 * The first two arguments, <tt>radius</tt> and <tt>sigma</tt>, specify the
	 * blurring used to create the "unsharp" image. See
	 * {@link #gaussianBlurImage} for a detail explanation. It will suffice to
	 * say that the larger the radius and sigma the more this blurred image will
	 * diverge from the original.
	 * <p>
	 * The last two arguments, <tt>threshold</tt> and <tt>amount</tt>, specify
	 * the difference threshold required to apply an adjustment to each pixel
	 * and, once the threshold is reached, the amount of the difference to be
	 * added back into the original pixels. A high threshold will cause the
	 * algorithm to only adjust edge pixels. Specifying a threshold will adjust
	 * every pixel.
	 *
	 * @param raduis
	 *            The radius of the gaussian, in pixels, not counting the center
	...
	public native MagickImage unsharpMaskImage(double raduis, double sigma,
			double amount, double threshold) throws MagickException;

	/**
	 * Creates a "ripple" effect in the image by shifting the pixels vertically
	 * along a sine wave whose amplitude and wavelength is specified by the
	 * given parameters.
	 *
	 * @param amplitude
	 *            Define the amplitude of the sine wave.
	 * @param wavelength
	 *            Define the wave-length of the sine wave.
	 *
	 ...
	 *
	 * @param x
	 *            x coordinate of the origin of the subimage
	 * @param y
	 *            y coordinate of the origin of the subimage
	 * @param width
	 *            width of the subimage
	 * @param height
	 *            height of the subimage
	 * @param map
	 *            component order of the pixels
	 * @param pixels
	...
	public native boolean dispatchImage(int x, int y, int width, int height,
			String map, byte[] pixels) throws MagickException;
	 ...
	 */
	public native boolean dispatchImage(int x, int y, int width, int height,
			String map, int[] pixels) throws MagickException;
	 ...
	 */
	public native boolean dispatchImage(int x, int y, int width, int height,
			String map, float[] pixels) throws MagickException;
	...
	public void setImageFormat(String imageFormat) throws MagickException {
	 ...
	 * @author Abdulbaset Gaddah <agaddah@yahoo.com>
	 */
	public native void setNumberColors(int numberColors) throws MagickException;
	...
	/**
	 * Creates a new image that is a rotated copy of an existing one. Positive
	 * angles rotate counter-clockwise (right-hand rule), while negative angles
	 * rotate clockwise. Rotated images are usually larger than the originals
	 * and have 'empty' triangular corners. X axis. Empty triangles left over
	 * from shearing the image are filled with the color defined by the pixel at
	 * location (0,0).
	 *
	 * @param degrees
	 *            angle of rotation.
	 ...
	 * y_shear is measured relative to the X axis. Empty triangles left over
	 * from shearing the image are filled with the color defined by the pixel at
	 * location (0,0).
	 *
	 * @param x_shear
	 *            x direction shear amount
	 * @param y_shear
	 *            y direction shear amount
	 ...
	 * minimize the difference between the input and output image while
	 * minimizing the processing time.
	 *
	 * @param quantizeInfo
	 *            contains parameters for quantization
	 ...
	 *
	 * @deprecated Use QuantizeInfo with MagickImage.quantizeImage to acheive
	 *             the same effect.
	 * @exception MagickException
	 *                on error
	 */
	...
	/**
	 * Creates a new image that is a copy of an existing one with the speckle
	 * noise minified. It uses the eight hull algorithm described in Applied
	...
	/**
	 * Applies a general image convolution kernel to an image returns the
	 * results. ConvolveImage allocates the memory necessary for the new Image
	...
	/**
	 * Searches the list of image attributes and returns the value of the
	 ...
	 * the list. SetImageAttribute returns True if the attribute is successfully
	 * replaced or added to the list, otherwise False. If the value is null, the
	 * matching key is deleted from the list.
	 *
	 * @param key
	 *            the key of the attribute
	 * @param value
	 ...
	 *
	 * @param imageInfo
	 *            a ImageInfo instance
	 * @param blob
	 *            memory containing an image in a known format
	 * @exception MagickException
	 *                on error
	 */
	 ...
	 *
	 * @param imageInfo
	 *            the magick member of this object determines output format
	 ...
	 *            the resolution type as defined in ResolutionType
	 * @see ResolutionType
	 * @exception MagickException
	 *                on error
	 */
	 ...
	 *
	 * @return A integer representing the resolution type as defined in
	 *         ResolutionType
	 * @see ResolutionType
	 * @exception MagickException
	 *                on error
	...
	/**
	 * Return image object for next image in sequence of frames. Also sets the
	 * next image pointer to NULL.
	 *
	...
	/**
	 * Destructively create array of image frames. Contains this image as the
	 * first object and frames in sequence.
	 *
	 ...
	 * 
	 * @param profile
	 *            a new color profile. This parameter cannot be null. Note: The
	 *            profile.name attribute is ignored
	 * @see #profileImage
	 * @throws MagickException
	 *             if an error occurs
	 */
	 ...
	 * 
	 * @param profile
	 *            a new IPTC profile. This parameter cannot be null. Note: The
	 *            profile.name attribute is ignored
	 * @see #profileImage
	 * @throws MagickException
	 *             if an error occurs
	 ...
	 * @throws MagickException
	 *             if the profile count cannot be retrieved
	 */
	 ...
	 * 
	 * @param i
	 *            the index of the generic profile to retrieve
	 ...
	 * image. If the profile name is defined it is deleted from the image. If a
	 * filename is given, one or more profiles are read and added to the image.
	 * 
	 * @param profileName
	 *            name of profile to add or remove
	 * @param profileData
	 ...
	 * 
	 * @param profileName
	 *            the profile name, for example icc, exif, and 8bim (8bim is the
	 ...
	 * 
	 * @param montageInfo
	 *            parameter used in the creation of the montage
	 ...
	 * Each image in the set must have the same width and height.
	 * 
	 ...
	 * 
	 * @param levels
	 *            String representing the black, mid and white levels
	 * @return true if successful, false otherwise.
	 */
	public native boolean levelImage(String levels) throws MagickException;
	...
	/**
	 * Set the compression attribute. This will affect a following call to
	 * writeImage
	 * 
	 * @see #writeImage
	 * @param value
	 *            a value from CompressionType
	 * @see CompressionType
	 * @see <a
	 *      href="http://www.imagemagick.org/script/command-line-options.php#compress">Command
	 *      line explanation</a>
	 * @exception MagickException
	 *                on error
	 */
	...
	/**
	 * Set the time in 1/100ths of a second (0 to 65535) which must expire
	 * before displaying the next image in an animated sequence. This option is
	 * useful for regulating the animation of a sequence of GIF images within
	 * Netscape.
	 * 
	 * @param delay
	 *            the time delay in 1/100th of a second
	 * @throws MagickException
	 *             upon error
	 */
	...
	/**
	 * Get the time in 1/100ths of a second (0 to 65535) which must expire
	 * before displaying the next image in an animated sequence. This option is
	 * useful for regulating the animation of a sequence of GIF images within
	 * Netscape.
	 * 
	 ...
	 * 
	 * @param dispose
	 *            the disposal method for GIF animation
	 * @throws MagickException
	 *             upon error
	 */
	public native void setDispose(int dispose) throws MagickException;
	...
	/**
	 * Set the number of iterations to loop an animation (e.g. Netscape loop
	 * extension) for.
	 * 
	 * @param iterations
	...
	/**
	 * Get the number of iterations to loop an animation (e.g. Netscape loop
	 * extension) for.
	 * 
	...
	/**
	 * Get the The number of colors in the image after QuantizeImage(), or
	 * QuantizeImages().
	 * 
	 * @return the current number of colors in the image
	 * @throws MagickException
	 ...
	 * 
	 * @param index
	 *            the index for which to return a colour
	 * @return the colour of the index
	 * @throws MagickException
	 *             upon error
	 */
	public native PixelPacket getColormap(int index) throws MagickException;
	 ...
	 * 
	 * @param x
	 *            the x position of the pixel to fetch
	 * @param y
	 *            the y position of the pixel to fetch
	 * @return the colour of the index
	 * @throws MagickException
	 *             upon error
	 */
	public native PixelPacket getOnePixel(int x, int y) throws MagickException;

	/**
	 * Determine signature of image and place signature in the image's
	 * attributes.
	 * 
	 * @return true if successful, false, otherwise
	 * @exception MagickException
	 ...
	 * 
	 * @param intent
	 *            the rendering Intent type as defined in RenderingIntent
	 * @see RenderingIntent
	 * @exception MagickException
	 *                on error
	 */
	 ...
	 * 
	 * @return A integer representing the rendering Intent type as defined in
	 *         RenderingIntent
	 * @exception MagickException
	 *                on error
	 */
	 ...
	 *                on error
	 */
	public native void setMatte(boolean matte) throws MagickException;
